...
function layout(container: HTMLElement, data: Data, width: number, height: number) {
  ...
  for (let start = 0; start < children.length;) {
    ...
    const {end, sum} = selectSpan(sizes, space, start);
    ...
    for (let i = start; i < end; i++) {
      ...
      layout(dom, children[i], width/scale, height/scale);
...
export function render(container: HTMLElement, data: any) {
  layout(container, data, container.offsetWidth, container.offsetHeight);
}